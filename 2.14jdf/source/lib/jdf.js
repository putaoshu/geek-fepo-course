...
jdf.init = function(argv) {
    ...
    jdf.getConfig(function(configData) {
        ...
        if (argv.length < 3 || cmd2 === '-h' || cmd2 === '--help') {
            ...
            if (cmd3 === 'init') {
                Log.send('wx-init');
                wx.init(cmd4 || 'mini-program-template');
            } else if (cmd3 === 'page') {
                Log.send('wx-page');
                wx.page(cmd4  || 'demo');
            }
        ...
        } else if (cmd2 === 'cp' || cmd2 === 'nbi') {
            var cpDirPath = jdf.config.output.cpDirPath || []

            if (!cpDirPath.length) {
                console.log('cpDirPath is empty!')
                return
            }

            var homeDir = require('os').homedir()

            cpDirPath.forEach(function(item) {
                var _userRoot = item.userRoot || homeDir

                if (!_userRoot) {
                    console.log('can not find userRoot!')
                    return
                }
                
                var _sourcePath = path.join(_userRoot, item.sourcePath)
                var _targetPath = path.join(_userRoot, item.targetPath)

                if (f.exists(_sourcePath) && f.exists(_targetPath)) {
                    console.log('cp -r ' + _sourcePath + ' to ' + _targetPath)

                    f.copy(_sourcePath, _targetPath)

                    console.log('cp success!')
                } else {
                    console.log('sourcePath or targetPath is not exist!')
                }
            })
        } else if (cmd2 === 'minner') {
            console.log('jdf minner upload start...');
            ...
            if (cmd3 && cmd4) {
                ...
                var serverDir = jdf.config.serverDir
                var projectPath = jdf.config.projectPath
                var splitPathArr = projectPath && projectPath.split('/');
                var zipName = splitPathArr && splitPathArr[splitPathArr.length - 1] + '.zip';

                source = jdf.currentDir + '/' + zipName
                dest = serverDir + projectPath
            }

            jdf.minner(source, dest, cmd5)
        }
         else {
...
/**
 * @上传zip包到预发，自解压到dest目录
 * @param {String}  source zip包路径
 * @param {Number}  dest 预发路径
 */
jdf.minner = function(source, dest, ip) {
    var request = require('request');
    var fs = require('fs');

    if (!f.exists(source)) {
        console.log('path ' + source + " not exist!");
        return;
    }
    ...
    var host = jdf.config.host || 'check.jr.jd.com'
    ...
    request.post({
        // url: 'http://check.jr.jd.com/upload', 
        url: url,
        formData: formData
        }, 
...
jdf.createStandardDir = function(dir) {
    ...
    fileArray[2] = '.editorconfig';
    fileArray[3] = '.eslintrc';
    fileArray[4] = '.prettierrc';
    fileArray[5] = '.eslintignore';
